...
public:
  ...
  {
    rgb_sub.subscribe(nh_, "/hsrb/head_rgbd_sensor/rgb/image_rect_color", 3);
    depth_sub.subscribe(nh_, "/hsrb/head_rgbd_sensor/depth_registered/image", 3);
    sync_input_2_ = new message_filters::Synchronizer<message_filters::sync_policies::ApproximateTime< sensor_msgs::Image,sensor_msgs::Image> >(3);
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cut_object_function_part(std::vector<geometry_msgs::Point> traj)
  {
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr function_cloud_rgb_record (new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::copyPointCloud(*cloud_cropped, *function_cloud_rgb_record);
	for(int i=0; i<function_cloud_rgb_record->points.size(); i++)
	  {
		function_cloud_rgb_record->points[i].r = 0;
		function_cloud_rgb_record->points[i].g = 0;
		function_cloud_rgb_record->points[i].b = 255;
	  }
	...
	*cloud_pub += *function_cloud_rgb;
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_record (new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::copyPointCloud(*cloud_remain, *cloud_record);
	for(int i=0; i<cloud_record->points.size(); i++)
	  {
		cloud_record->points[i].r = 255;
		cloud_record->points[i].g = 0;
		cloud_record->points[i].b = 0;
	  }

	*cloud_record += *function_cloud_rgb_record;

	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_transformed = cloud_transform_to_origin(cloud_pub);


	ply_saver.write("activated_object.ply",*cloud_transformed);    

	return cloud_cropped;
  }

  pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_transform_to_origin(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud)
  {
  ...
  void publish_text(geometry_msgs::Point location)
  {
	...
	marker.id = 0;
	marker.text = "Recorded Activated Object";
	...
	if(data=="finish")
	  {
		...
		int traj_len = traj.size();
		publish_text(traj[traj_len-1]);	
	  }
  }
