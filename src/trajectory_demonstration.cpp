...
public:
  ...
  trajectory_demonstration():it_(nh_)//:camera_info{0}
  {
	...
	if(data=="hand_stable" && found_obj1_flag && found_obj2_flag)
	  {
		...
		for(int i=0; i<associate_cloud_show->points.size(); i++)
		  {
			associate_cloud_show->points[i].r = 80;
			associate_cloud_show->points[i].g = 80;
			associate_cloud_show->points[i].b = 255;
		  }
	...
	if(data=="hand_moving" && !got_trajectory_flag)
	  {
		...
		if (activated_object_trajectory.size()<10)
		  {
			activated_object_trajectory.push_back(activated_object);
			cout<<"!!!!!!!!!!! trajectory length "<<activated_object_trajectory.size()<<endl;
		  }
		else
		  {
			//icp
			...
			std::vector<float> start_pose{activated_object_Centroid(0),activated_object_Centroid(1),activated_object_Centroid(2),1.0,0.0,0.0,0.0};
			...
			for(int i=1; i<activated_object_trajectory.size(); i++)
			  {
				...
				cout<<"x "<<transformed_pose[0]<<" y "<<transformed_pose[1]<<" z "<<transformed_pose[2]<<endl;
				...
				for(int i=0; i<obj_pair_cloud_show->points.size(); i++)
				  {
					...
					obj_pair_cloud_show->points[i].g = 255;
					obj_pair_cloud_show->points[i].b = 160;
				  }
