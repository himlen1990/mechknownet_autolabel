...
public:
  ...
  trajectory_demonstration():it_(nh_),labelled_cloud(new pcl::PointCloud<pcl::PointXYZRGB>)//:camera_info{0}
  {
    ...
    sync_input_2_ = new message_filters::Synchronizer<message_filters::sync_policies::ApproximateTime< sensor_msgs::Image,sensor_msgs::Image> >(1);//queue size
  ...
  void publish_text(std::vector<float> location)
  {
	...
	marker.action = visualization_msgs::Marker::ADD;
	marker.id = 0;
	marker.text = "Learned Function: Cutting";
